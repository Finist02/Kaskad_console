...
export class SystemTreeProvider implements vscode.TreeDataProvider<SystemItem> {
    ...
    private getValidVersion(project: ProjectInfo): string {
        ...
        try {
            ...
            throw new Error(
                `Cannot determine Kaskad version for project ${project.id}: ${err.message}`,
            );
        }
    }
    ...
    async startProject(project: ProjectInfo): Promise<void> {
        ...
        try {
            ...
            ExtensionOutputChannel.debug(
                'SystemTreeProvider',
                `Setting Kaskad version: ${version}`,
            );
            this.pmon.setVersion(version);

            ExtensionOutputChannel.showInfoNotification(`⟳ Starting ${project.name}...`);
            ...
            if (pmonStatus !== ProjEnvPmonStatus.Running) {
                ...
                ExtensionOutputChannel.showInfoNotification(`⟳ Starting PMON for ${project.name}...`);
                ExtensionOutputChannel.info(
                    'SystemTreeProvider',
            ...
            if (result === 0) {
                ExtensionOutputChannel.showInfoNotification(`✓ ${project.name} started successfully`);
    ...
    async stopProject(project: ProjectInfo): Promise<void> {
        ...
        if (answer === 'Yes') {
            try {
                ...
                ExtensionOutputChannel.showInfoNotification(`⏹ Stopping ${project.name}...`);
                ...
                ExtensionOutputChannel.showInfoNotification(`⏹ Stopping PMON for ${project.name}...`);
                ExtensionOutputChannel.info(
                    'SystemTreeProvider',
                ...
                if (pmonResult === 0) {
                    ExtensionOutputChannel.showInfoNotification(`✓ ${project.name} stopped`);
                    await this.projectManager.refreshProjects();
                    this.refresh();
                } else {
                    vscode.window.showErrorMessage(
    ...
    async startOASystem(): Promise<void> {
        ...
        try {
            ...
            ExtensionOutputChannel.debug(
                'SystemTreeProvider',
                `Setting Kaskad version: ${currentProject.version}`,
            );
            this.pmon.setVersion(currentProject.version);

            ExtensionOutputChannel.showInfoNotification(`⟳ Starting system for ${currentProject.name}...`);
            ...
            if (result === 0) {
                ExtensionOutputChannel.showInfoNotification(`✓ System started for ${currentProject.name}`);
    ...
    async setActiveProject(project: ProjectInfo): Promise<void> {
        ...
        try {
            ...
            ExtensionOutputChannel.showInfoNotification(`✓ ${project.name} set as active project`);
            this.refresh();
        } catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
    ...
    async addProjectToWorkspace(project: ProjectInfo): Promise<void> {
        ...
        try {
            ...
            if (alreadyInWorkspace) {
                ExtensionOutputChannel.showInfoNotification(`Project already in workspace`);
                return;
            }
            ...
            if (success) {
                ExtensionOutputChannel.showInfoNotification(`✓ Project added to workspace`);
            } else {
    ...
    async addSubprojectToWorkspace(subprojectPath: string): Promise<void> {
        ...
        try {
            ...
            if (alreadyInWorkspace) {
                ExtensionOutputChannel.showInfoNotification(`Subproject already in workspace`);
                return;
            }
            ...
            if (success) {
                ExtensionOutputChannel.showInfoNotification(`✓ Subproject added to workspace`);
            } else {
    ...
    async restartOASystem(): Promise<void> {
        ...
        if (answer === 'Yes') {
            try {
                // Set Kaskad version for pmon component
                this.pmon.setVersion(currentProject.version);
    ...
    /**
     * Register a Kaskad project from Explorer context menu
     */
    ...
    async registerNewProject(): Promise<void> {
        try {
            ...
            const projectFolder = await vscode.window.showOpenDialog({
                ...
                title: 'Select Kaskad Project to Register',
            });
            ...
            vscode.window.withProgress(
                ...
                async (progress) => {
                    try {
                        ...
                        progress.report({ message: 'Registering with Kaskad...' });
                        ExtensionOutputChannel.info(
                            'SystemTreeProvider',
    ...
    async unregisterProject(projectData: ProjectInfo): Promise<void> {
        ...
        const confirm = await vscode.window.showWarningMessage(
            `⚠️ Unregister Project "${projectId}"?\n\nThis will remove the project from Kaskad's registry (pvssInst.conf).\nThe project files will NOT be deleted.`,
            { modal: true },
            'Yes, Unregister',
            'Cancel',
        );
        ...
        try {
            await vscode.window.withProgress(
                ...
                async (progress) => {
                    ...
                    // Unregister project
                    progress.report({ message: 'Unregistering from Kaskad...' });
                    ExtensionOutputChannel.info(
                        'SystemTreeProvider',
