...
export class ManagerTreeProvider implements vscode.TreeDataProvider<ManagerItem> {
    ...
    private async loadManagers(): Promise<void> {
        ...
        try {
            ...
            // Set Kaskad version for pmon component (already parsed in toProjectInfo())
            ...
            ExtensionOutputChannel.debug(
                'ManagerTreeProvider',
                `getProjectStatus returned ${projectStatus?.managers?.length ?? 0} managers`,
            );
            ...
            ExtensionOutputChannel.debug(
                'ManagerTreeProvider',
                `getManagerOptionsList returned ${managerOptions?.length ?? 0} options`,
            );
    ...
    async getChildren(element?: ManagerItem): Promise<ManagerItem[]> {
        if (!element) {
            ...
            // Root level - show managers directly as a flat list
            ...
            return this.managers.map((mgr) => {
                ...
                const num = mgr.info.managerNumber ?? mgr.idx;
                const label = `${componentName} (${num})`;
    ...
    async startManager(managerData: ManagerDisplayData): Promise<void> {
        ...
        try {
            ...
            if (result === 0) {
                ExtensionOutputChannel.showInfoNotification(`✓ Manager started successfully`);
            } else {
    ...
    async stopManager(managerData: ManagerDisplayData): Promise<void> {
        ...
        try {
            ...
            if (result === 0) {
                ExtensionOutputChannel.showInfoNotification(`✓ Manager stopped successfully`);
            } else {
    ...
    async restartManager(managerData: ManagerDisplayData): Promise<void> {
        ...
        try {
            ExtensionOutputChannel.showInfoNotification('⟳ Restarting manager...');
            ...
            if (startResult === 0) {
                ExtensionOutputChannel.showInfoNotification(`✓ Manager restarted successfully`);
            } else {
                vscode.window.showErrorMessage(
    ...
    async addManager(): Promise<void> {
        ...
        try {
            ...
            if (typeSelection.label === 'Custom') {
                ...
                // For Kaskad managers, use the full name without suffix
            ...
            // Suggest -num X for Kaskad managers
            let defaultOptions = '';
            if (typeSelection.label !== 'Custom') {
...
function formatStartMode(mode: unknown): string {
    if (typeof mode === 'string') return mode;
    if (mode === ProjEnvManagerStartMode.Manual || mode === 0) return 'manual';
    if (mode === ProjEnvManagerStartMode.Once || mode === 1) return 'once';
    if (mode === ProjEnvManagerStartMode.Always || mode === 2) return 'always';
    return '—';
}
...
class ManagerItem extends vscode.TreeItem {
    ...
    constructor(
        ...
        if (itemType === 'folder') {
            ...
            const opts = managerData.options;
            const startModeStr = formatStartMode(info.startMode ?? opts?.startMode);
            const optsStr = startOptions || opts?.startOptions || '—';
            ...
            tooltip.appendMarkdown(`| **Start mode** | ${startModeStr} |\n`);
            if (optsStr !== '—') {
                const escaped = optsStr.replace(/\|/g, '\\|').replace(/`/g, '\\`');
                tooltip.appendMarkdown(`| **Options** | \`${escaped}\` |\n`);
            }
            tooltip.appendMarkdown(`| Index | ${managerData.idx} |\n`);
            tooltip.appendMarkdown(`| PID | ${info.pid ?? '—'} |\n`);
            if (info.startTime && info.startTime.getTime?.() !== 0) {
                tooltip.appendMarkdown(`| Start time | ${info.startTime} |\n`);
            }
