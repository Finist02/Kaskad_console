/**
 * @fileoverview Core logging utility for Kaskad VS Code extensions.
...
/**
 * Core logging utility class for Kaskad VS Code extensions.
 *
...
export class ExtensionOutputChannel {
    ...
    /**
     * Shows informational popup (toast) only when kaskadProjectAdmin.showInfoNotifications is enabled.
     * Use for success/progress messages. Errors and confirmations always show.
     */
    public static showInfoNotification(message: string): void {
        const config = vscode.workspace.getConfiguration(EXTENSION_CONFIG_SECTION);
        if (config.get<boolean>('showInfoNotifications', false)) {
